...
import json
import sys
from mcp_word import run_word_extraction
...
@app.tool()
def extract_word(file_path: str, schema_json: Optional[str] = None, model: str = "meta-llama/llama-4-maverick-17b-128e-instruct") -> Dict[str, Any]:
    """
    Extract structured data from a Word document using run_word_extraction in mcp_word.py.
    Args:
        file_path: path to .docx file
        schema_json: optional JSON schema string (or path)
    ...
    try:
        result = run_word_extraction(file_path, schema, model, push_to_mcp_flag=False)
        return result
    except Exception as e:
        return {"status": "error", "message": f"Word extraction failed: {str(e)}"}
